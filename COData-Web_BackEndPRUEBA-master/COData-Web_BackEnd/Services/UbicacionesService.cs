...
namespace COData_Web_BackEnd.Services
{
    public class UbicacionesService : IUbicacionesServices
    {
        ...
        public Ubicaciones InsertUbicacion(Ubicaciones ubicacion)
        {
            ...
            // Manejo profesional de nulos para ADO.NET
            cmd.Parameters.AddWithValue("@Direccion", (object)ubicacion.Direccion ?? DBNull.Value);
            cmd.Parameters.AddWithValue("@Ciudad", (object)ubicacion.Ciudad ?? DBNull.Value);
            cmd.Parameters.AddWithValue("@Latitud", ubicacion.Latitud);
            cmd.Parameters.AddWithValue("@Longitud", ubicacion.Longitud);

            conn.Open();

            // Cambiamos ExecuteNonQuery por ExecuteScalar para atrapar el SCOPE_IDENTITY
            var result = cmd.ExecuteScalar();
            if (result != null && result != DBNull.Value)
            {
                ubicacion.UbicacionId = Convert.ToInt32(result);
            }
