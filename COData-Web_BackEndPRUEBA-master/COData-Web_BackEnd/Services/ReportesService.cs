...
namespace COData_Web_BackEnd.Services
{
    public class ReportesService : IReportesService
    {
        ...
        public Reportes InsertReporte(Reportes reporte)
        {
            ...
            // Atrapar el SCOPE_IDENTITY
            var result = cmd.ExecuteScalar();
            if (result != null && result != DBNull.Value)
            {
                reporte.ReporteId = Convert.ToInt32(result);
            }

            return reporte;
        }

        private bool UbicacionExists(int ubicacionId)
        {
            using SqlConnection conn = new SqlConnection(_connectionString);
            using SqlCommand cmd = new SqlCommand(
                "SELECT COUNT(1) FROM Ubicaciones WHERE UbicacionId = @UbicacionId", conn);
